...
func (r *Resolver) ResolveByName(ctx context.Context, gatewayName string) (domain.PaymentGateway, error) {
	if r == nil || r.priority == nil {
		return nil, fmt.Errorf("gateway resolver not configured")
	}
	...
	return nil, fmt.Errorf("no payment gateway available / found in priority list")
}
